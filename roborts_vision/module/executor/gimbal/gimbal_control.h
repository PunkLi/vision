...
class GimbalContrl
{
 ...
 public:
  GimbalContrl()
  {
    std::string file_name = ros::package::getPath("roborts_vision") + "/module/executor/gimbal/gimbal.xml";
    
    cv::FileStorage fs(file_name, cv::FileStorage::READ);
    if(!fs.isOpened())
        ROS_ERROR ("Cannot open gimbal config file, please check if the file is exist.");
  ...
  void UpdateFricSpeed(float fric_speed)
  {
    if(12 < fric_speed && fric_speed <= 30) 
    {
      init_v_ = fric_speed;
      v_reliable_ = init_v_;
    }
