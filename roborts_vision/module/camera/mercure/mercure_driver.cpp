...
MercureDriver::MercureDriver(int idx):
...
GX_STATUS MercureDriver::init_sdk(int index)
{
    ...
    status_ = GXOpenDeviceByIndex(index, &device_);
...
void MercureDriver::LoadParam()
{
    ...
    ros::NodeHandle nh("~");
#else
    std::string file_name = ros::package::getPath("roborts_vision") + "/module/camera/mercure/mercure.xml";
