...
{	
	hyy_detection::ArmorDetector hyy_detector;
/*
...
*/
		...
		hyy_detector.ArmorFinder(src_img_);
		if(hyy_detector.armors.size())
    {
      hyy_detector.ArmorFliter(hyy_detector.armors);


    	/*to draw Rrect*/
    	for (uchar i = 0; i < hyy_detector.armors.size(); i++)
      {
        cv::Point2f* vertices = new cv::Point2f[4];
        hyy_detector.armors[i].points(vertices);
        for (uchar j = 0; j < 4; j++)
        {
          cv::line(src_img_, vertices[j], vertices[(j + 1) % 4], cv::Scalar(0, 255, 0), 8);
        }

      }

			
			// armors[0].rect = hyy_detector.armors[0];
			Armor_Twist armor_twist = MID_MOVE;
			ArmorInfo temp_armor(hyy_detector.armors[0], armor_twist);
			armors.push_back(temp_armor);
    }
		cv::imshow("rect_img", src_img_);
		
		// armors[0].rect = hyy_detector.armors[0];
		// armors[0].Armor_Twist = MID_MOVE;
		// armors[0].ArmorType = SMALLARMOR;
		// armors[0] = temp_armor;
		// armors[0].rect = hyy_detector.armors[0];
		// for (int i = 0; i != armors.size(); ++i)
		// if(hyy_detector.armors.empty())
    // cv_toolbox_->DrawRotatedRect(src_img_, hyy_detector.armors[0], cv::Scalar(0,255,0), 2);
